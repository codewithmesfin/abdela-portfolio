<template>
    ...
    <div class="md:w-[60%] mx-auto px-5 md:px-0">
        <h2
            class="md:text-6xl pb-3 leading-6 text-center md:text-left  text-xl font-bold sm:text-4xl">
            Top Notch Support
        </h2>
        ...
        <div class="md:grid grid-cols-2 gap-4 ">
            <div v-for="(tip, i) in tips" :key="i"
                class="block my-5 md:my-2 flex items-center cursor-pointer p-2 md:p-6 max-w-sm rounded-lg md:border  md:shadow-md card"
                >
               <div>
                <p class="text-xl text-center font-extrabold"> {{ tip.title }}</p>
                <p class=" py-2 text-center">
                    <span class="two-line">{{ tip.desc }}</span>
                    <RouterLink to="/services" class="text-blue-600">Learn more</RouterLink>
                </p>
               </div>
            </div>
        </div>
